...
let clustersPromise: Promise<{ points: ClusterPoint[]; summary: ClusterSummary[] }> | null = null;

let dataCache: Rec[] | null = null;
let rulesCache: Rule[] | null = null;
let clustersCache: { points: ClusterPoint[]; summary: ClusterSummary[] } | null = null;

export function preloadAll() {
  if (!dataPromise) dataPromise = fetchData().then((d) => (dataCache = d));
  if (!rulesPromise) rulesPromise = fetchRules().then((d) => (rulesCache = d));
  if (!clustersPromise) clustersPromise = fetchClusters().then((d) => (clustersCache = d));
}
...
export function useDataset() {
  return useCached<Rec[]>(
    () => dataCache,
    () => (dataPromise ??= fetchData().then((d) => (dataCache = d))),
  );
}
export function useRules() {
  return useCached<Rule[]>(
    () => rulesCache,
    () => (rulesPromise ??= fetchRules().then((d) => (rulesCache = d))),
  );
}
export function useClusters() {
  return useCached<{ points: ClusterPoint[]; summary: ClusterSummary[] }>(
    () => clustersCache,
    () => (clustersPromise ??= fetchClusters().then((d) => (clustersCache = d))),
  );
}